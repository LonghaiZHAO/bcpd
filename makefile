CC=gcc
BCPDSRC=  register/*.c base/*.c base/rply/rply.c
...
ifeq ($(OPT),-DUSE_OPENMP)
  ifeq ($(ENV),LINUX)
	...
	$(CC) -O3 -fopenmp $(OPT) $(DEBUG) $(INCLUDES) $(BCPDSRC) win/*.dll -o win/bcpd -lm -DMINGW32
  else ifeq ($(ENV),HOMEBREW_INTEL)
	clang -O3 $(OPT) $(OMP_BREW_ITL) $(DEBUG) $(INCLUDES) $(BCPDSRC) -o bcpd -lm -llapack -Wuninitialized
  else ifeq ($(ENV),HOMEBREW)
	clang -O3 $(OPT) $(OMP_BREW_ARM) $(DEBUG) $(INCLUDES) $(BCPDSRC) -o bcpd -lm -llapack -Wuninitialized
  else ifeq ($(ENV),MACPORTS)
	clang -O3 $(OPT) $(OMP_PORT) $(DEBUG) $(INCLUDES) $(BCPDSRC) -o bcpd -lm -llapack -Wuninitialized
  endif
else
  ifeq ($(OPT),-DNUSE_OPENMP)
	$(CC) -O3 $(DEBUG) $(INCLUDES) $(BCPDSRC) -o bcpd -lm -llapack
  else ## default ##
	clang -O3 -DUSE_OPENMP $(OMP_BREW_ARM) $(DEBUG) $(INCLUDES) $(BCPDSRC) -o bcpd -lm -llapack
  endif
endif
